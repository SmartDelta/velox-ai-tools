...
| `velox-ai detect --weights W --frames frames.json --out dets.json [--tile 1280 --overlap 0.2 --imgsz 1280 --conf 0.25 --batch 4 --device auto --classes 0 2 --resume]` | Tiled detection for a list of frames. Input: `[{"id", "img_path"}]`. Output per frame: width, height and detections `{"cls", "cls_id", "conf", "box": [x1,y1,x2,y2] px, "poly"}`. `--tile 0` = whole frame. Every finished frame is appended to `<out>.partial.jsonl` at once, so a killed run can continue with `--resume` (done frames are kept and skipped). |
